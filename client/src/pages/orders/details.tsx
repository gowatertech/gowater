...
export default function OrderDetails() {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        header.innerHTML = `
          ...
          <div style="font-size: 11px; margin-bottom: 2px;">Tel: ${phone}</div>
          <div style="font-size: 11px; margin-bottom: 2px;">Email: ${email}</div>
          <div style="font-size: 11px; margin-bottom: 5px;">RNC: ${rnc}</div>
        `;
        ...
        orderInfo.innerHTML = `
          <div style="margin-bottom: 5px;"><strong>Fecha:</strong> ${orderDate}</div>
          <div style="margin-bottom: 5px;"><strong>Cliente:</strong> ${businessName}</div>
          <div style="margin-bottom: 5px;"><strong>Teléfono:</strong> ${customerPhone}</div>
          <div style="margin-bottom: 5px;"><strong>Dirección:</strong> ${customerAddress}</div>
          <div style="margin-bottom: 5px;"><strong>Municipio:</strong> ${municipalityName}</div>
          <div style="margin-bottom: 5px;"><strong>Provincia:</strong> ${provinceName}</div>
          <div style="margin-bottom: 5px;"><strong>Método de pago:</strong> ${paymentMethod}</div>
          <div style="margin-bottom: 5px;"><strong>Estado:</strong> ${status}</div>
        `;
        ...
        if (Array.isArray(orderItems) && orderItems.length > 0) {
          orderItems.forEach((item: any) => {
            ...
            tableHTML += `
              <tr style="border-bottom: 1px solid #eee;">
                ...
                <td style="padding: 5px; text-align: right;">${quantity}</td>
                <td style="padding: 5px; text-align: right;">RD$ ${price.toFixed(2)}</td>
          ...
          tableHTML += `
            <tr>
              <td colspan="4" style="padding: 8px; text-align: center;">No hay productos para mostrar</td>
            </tr>
          `;
        }
        ...
        if (notes) {
          ...
          notesSection.innerHTML = `
            ...
            <div style="font-style: italic;">${notes}</div>
          `;
  ...
  const handleDownload = () => {
    try {
      ...
      const orderId = order.id || 'N/A';
      const orderDate = order.date ? new Date(order.date).toLocaleDateString() : 'Fecha no disponible';
      const businessName = currentCustomer?.businessname || "Cliente";
      const customerPhone = order.customerPhone || "No disponible";
      const customerAddress = order.customerAddress || "No especificada";
      const municipalityName = order.municipalityName || "No especificado";
      const provinceName = order.provinceName || "No especificada";
