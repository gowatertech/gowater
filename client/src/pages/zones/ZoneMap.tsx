...
function DrawingControl({ onPolygonComplete, disabled = false }: DrawingControlProps) {
...
export default function ZoneMap({ 
  ...
  return (
    <div className="flex flex-col gap-0.5">
      {/* Formulario ultracompacto, solo se muestra si no es vista previa */}
      {!viewOnly && (
        <div className="grid grid-cols-2 gap-1 mb-0.5">
          <div>
            ...
            <input
              ...
              type="text"
              value={newZoneName}
              onChange={(e) => setNewZoneName ? setNewZoneName(e.target.value) : undefined}
      ...
      <ResponsiveMapContainer className="bg-white border rounded-md" fixedHeight minHeight="180px">
        ...
        >
          ...
          <DrawingControl onPolygonComplete={handlePolygonComplete} disabled={viewOnly} />
          
          {/* Mostrar zonas existentes */}
          <ZonePolygons 
            zones={viewZoneId ? zones.filter(z => z.id === viewZoneId) : zones} 
            showLabels={!viewOnly} 
          />
          
          {/* Mostrar clientes en el mapa */}
          {!viewOnly && customers.map((customer) => {
