...
import { type Route, type Order } from "@shared/schema";
...
export default function Routes() {
  ...
  const startRouteMutation = useMutation({
    mutationFn: async (routeId: number) => {
      const response = await apiRequest("POST", `/api/routes/${routeId}/start`, {
        currentLocation: "-69.8734,18.4955" // Santo Domingo
      });
      return response.json();
    },
    onSuccess: () => {
  ...
  const completeRouteMutation = useMutation({
    mutationFn: async (routeId: number) => {
      const response = await apiRequest("POST", `/api/routes/${routeId}/complete`, {
        currentLocation: "-69.8734,18.4955" // Santo Domingo
      });
      return response.json();
    },
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <Dialog>
          <DialogTrigger asChild>
            <Button className="w-full sm:w-auto text-base">{t("optimizeRoute")}</Button>
          </DialogTrigger>
          <DialogContent className="w-[98vw] sm:w-[90vw] max-w-2xl p-2 sm:p-4 gap-3">
            <DialogHeader>
              <DialogTitle className="text-lg">{t("selectOrders")}</DialogTitle>
            </DialogHeader>
                    ...
                    <TableCell>
                      ...
                      {route.status === "in_progress" && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => completeRouteMutation.mutate(route.id)}
                        >
                          {t("complete")}
                        </Button>
                      )}
