...
export default function DriverRoute() {
  ...
  // Función para alternar la expansión de una parada
  const toggleExpandStop = (stopId: number) => {
    setExpandedStopId(expandedStopId === stopId ? null : stopId);
  };
  
  // Función para manejar la marcación de una parada como completada
  // Función para manejar la entrega de órdenes
  const handleDeliverOrder = (stop: RouteStop) => {
    console.log("Entregar pedido:", stop.id);
    // Abrir el diálogo de pago
    openPaymentDialog(stop);
  };
  
  const handleMarkStopCompleted = (stop: RouteStop) => {
    console.log("Marcando parada como completada:", stop.id);
    // Simplemente delegamos a la función de entregar pedido que ya existe
    handleDeliverOrder(stop);
  };
  
  // Función para editar un pedido
  const handleEditOrder = (stop: RouteStop) => {
    console.log("Editando pedido:", stop.id);
    // Por ahora, simplemente mostramos una notificación de que esta funcionalidad está en desarrollo
    toast({
  ...
  const handleViewOrderDetails = (stop: RouteStop) => {
    console.log("Viendo detalles del pedido:", stop.id);
    // Si es un pedido completado, podríamos mostrar un recibo o factura
    if (stop.status === 'completed') {
      // Por ahora simplemente mostramos un mensaje
      toast({
        title: "Funcionalidad en desarrollo",
        description: "La generación de facturas PDF está en desarrollo",
        variant: "default"
      });
    } else {
      // Si no está completado, simplemente expandimos/colapsamos la vista
      toggleExpandStop(stop.id);
    }
  };
  
  // Función para manejar el retorno de envases
  const handleBottleReturn = (orderId: number) => {
    console.log("Registrando retorno de envases para el pedido:", orderId);
    setCurrentOrderIdForReturn(orderId);
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      const updateData = {
        ...
        createInvoice: true, // Siempre crear factura al completar entrega
        printInvoice: false  // Por defecto no imprimir, se puede habilitar después
      };
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2 mt-1">
                <Button 
                  ...
                  onClick={() => viewInvoice(responseData.invoiceId)}
                >
                ...
                <Button 
                  ...
                  variant="outline" 
                  className="py-0 h-7 text-xs"
                  onClick={() => printInvoice(responseData.invoiceId)}
                >
  ...
  const viewInvoice = (invoiceId: number) => {
    ...
    window.open(`/mobile-app/facturas/${invoiceId}`, '_blank');
  };
  ...
  const printInvoice = async (invoiceId: number) => {
    try {
      ...
      const doc = new jsPDF({
        ...
        unit: 'mm',
        format: [80, 200] // Tamaño típico para impresora térmica
      });
      
      // Configurar el documento
      doc.setFontSize(10);
      doc.text("GOWATER", 40, 10, { align: 'center' });
      doc.setFontSize(8);
      doc.text("Factura de compra", 40, 15, { align: 'center' });
      doc.text(`Factura #: ${invoiceData.id}`, 10, 25);
      doc.text(`Fecha: ${new Date(invoiceData.date).toLocaleDateString()}`, 10, 30);
      doc.text(`Cliente: ${invoiceData.customerName}`, 10, 35);
      ...
      // Agregar productos
      invoiceData.items.forEach((item: { name: string; quantity: number; total: string | number }) => {
        doc.text(item.name.substring(0, 25), 10, y);
        doc.text(item.quantity.toString(), 50, y);
        doc.text(`$${parseFloat(item.total.toString()).toFixed(2)}`, 65, y);
        y += 5;
      });
      ...
      doc.setFontSize(10);
      doc.text("Total:", 45, y);
      doc.text(`$${parseFloat(invoiceData.total).toFixed(2)}`, 65, y);
      
      // Método de pago
      y += 10;
      doc.setFontSize(8);
      doc.text(`Forma de pago: ${invoiceData.paymentMethod === 'cash' ? 'Efectivo' : 'Crédito'}`, 10, y);
      
      // Mensaje de agradecimiento
      y += 15;
      doc.text("¡Gracias por su compra!", 40, y, { align: 'center' });
      
      // Abrir o imprimir el PDF
  ...
  // Función para abrir diálogo de creación de nuevo pedido
  const openCreateNewOrderDialog = (customerId: number) => {
    // Redirigir a la pantalla de creación de pedidos con el cliente preseleccionado
    setLocation(`/mobile-app/pedidos/nuevo?cliente=${customerId}`);
  };
