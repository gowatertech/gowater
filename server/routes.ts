...
export async function registerRoutes(app: Express) {
    ...
    try {
      ...
      const [loading] = await db
        ...
        .values({
          ...
          initialCash: result.data.initialCash,
          notes: result.data.notes,
        })
        .returning();

      // Crear los items de la carga
  ...
  app.get("/api/vehicle-loading", async (req, res) => {
    try {
      const loadings = await db
        .select()
        .from(vehicleLoading)
        .orderBy(vehicleLoading.date);

      console.log("GET /api/vehicle-loading - Retornando:", loadings.length, "cargas");
      res.json(loadings);
    } catch (error) {
  ...
  app.put("/api/vehicle-loading/:id", async (req, res) => {
    try {
      const loadingId = parseInt(req.params.id);
      console.log("PUT /api/vehicle-loading/:id - Body recibido:", req.body);
      ...
      console.log("PUT /api/vehicle-loading/:id - Carga actualizada:", loading);
      res.json(loading);
    } catch (error) {
