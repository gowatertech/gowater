...
import { apiRequest } from './api'; // Assuming this function exists
...
const NewOrder = () => {
  ...
  const addItem = (item) => {
    setOrderData({ ...orderData, items: [...orderData.items, item] });
  };
  ...
  const createOrder = async () => {
    ...
    if (!orderData.items?.length || !orderData.items.every(item =>
      item.productId &&
